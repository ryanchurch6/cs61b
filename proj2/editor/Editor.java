...
public class Editor extends Application {
    ...
    private class KeyEventHandler implements EventHandler<KeyEvent> {
        ...
        private static final double STARTING_TEXT_POSITION_X = 0;
        private static final double STARTING_TEXT_POSITION_Y = 0;
	Grid parent = new Grid(STARTING_FONT_SIZE, WINDOW_WIDTH, WINDOW_HEIGHT);

        /** The Text to display on the screen. */
        private Text currentText = new Text(STARTING_TEXT_POSITION_X, STARTING_TEXT_POSITION_Y, "");
        ...
        public void handle(KeyEvent keyEvent) {
            if (keyEvent.getEventType() == KeyEvent.KEY_TYPED) {
                ...
                if (characterTyped.length() > 0 && characterTyped.chatAt(0) == 8) {
                    // Ignore control keys, which have non-zero length, as well as the backspace
                    // key, which is represented as a character of value = 8 on Windows.
		    // CHANGE TO BE MADE: we need (List + characterTyped) rather than just characterTyped.
